...
const handleViewClick = (user) => {
  ...
  const { profile, loading: profileLoading } = useProfile();
   ...
   const [hasFetchedAndChecked, setHasFetchedAndChecked] = useState(false);
...
const fetchUsers = async (page: number, limit: number, search: string = "") => {
  try {
    setLoading(true);
    if (!profile) return;
    console.log("allclassestable email",profile?.user?.email)
    const api = process.env.NEXT_PUBLIC_API;
    const response = await fetch(`${api}/classmanagement/all?page=${page}&limit=${limit}&search=${search}&userEmail=${profile?.user?.email}`, {
      headers: authHeader()
    });
    ...
    setLoading(false);
    setHasFetchedAndChecked(true);
  }
};

  // Initial fetch and when page/limit/search changes
 useEffect(() => {
    // Only fetch if profile has loaded
    if (!profileLoading) {
      fetchUsers(currentPage, itemsPerPage, searchTerm);
    }
  }, [currentPage, itemsPerPage, searchTerm, profile, profileLoading]);
  ...
  }, [searchTerm, itemsPerPage,profile, profileLoading]);



useEffect(() => {
    if (hasFetchedAndChecked && classes.length === 0) {
      router.push('/classmanagement/add');
    }
  }, [hasFetchedAndChecked, classes, router]);
  ...
  const isLoading = profileLoading || (loading && !hasFetchedAndChecked);

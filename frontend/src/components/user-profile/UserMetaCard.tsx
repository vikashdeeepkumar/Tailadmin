...
export default function UserMetaCard({ profile,onProfileUpdate }: UserMetaCardProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    facebook_profile: profile?.user?.facebook_profile || "",
    x_com_profile: profile?.user?.x_com_profile || "",
    linkedin_profile: profile?.user?.linkedin_profile || "",
    instagram_profile: profile?.user?.instagram_profile || "",
...
const handleInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  //     return `/images/user/${profile.user.avatar}`;
  //   }
  ...
  if (!url) {
    return undefined; // Hide the link if no URL is present
  }
  // Check if the URL already has a protocol (like http:// or https://)
  if (!/^(https?|ftp):\/\//i.test(url)) {
    // If not, prepend a protocol. https is generally preferred.
    return `https://${url}`;
  }
  return url;
};
      ...
      <div className="p-5 border border-gray-200 rounded-2xl dark:border-gray-800 lg:p-6">
        <div className="flex flex-col gap-5 xl:flex-row xl:items-center xl:justify-between">
          <div className="flex flex-col items-center w-full gap-6 xl:flex-row">
            <div className="flex-shrink-0 w-20 h-20 overflow-hidden border border-gray-200 rounded-full dark:border-gray-800">
              <Image
                width={80}
                height={80}
               alt="user avatar"
            ...
            <div className="flex items-center order-2 gap-2 grow xl:order-3 xl:justify-end">
              <a        
                target="_blank"
                rel="noreferrer" href={normalizeUrl(profile?.user?.facebook_profile)} className="flex h-11 w-11 items-center justify-center gap-2 rounded-full border border-gray-300 bg-white text-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200">
                <svg
              ...
              <a href={normalizeUrl(profile?.user?.x_com_profile)} target="_blank"
        rel="noreferrer"  className="flex h-11 w-11 items-center justify-center gap-2 rounded-full border border-gray-300 bg-white text-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200">
              ...
              <a href={normalizeUrl(profile?.user?.linkedin_profile)} target="_blank"
        rel="noreferrer" className="flex h-11 w-11 items-center justify-center gap-2 rounded-full border border-gray-300 bg-white text-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200">
                <svg
                  className="fill-current"
                  width="20"
              ...
              <a href={normalizeUrl(profile?.user?.instagram_profile)} target="_blank"
        rel="noreferrer" className="flex h-11 w-11 items-center justify-center gap-2 rounded-full border border-gray-300 bg-white text-sm font-medium text-gray-700 shadow-theme-xs hover:bg-gray-50 hover:text-gray-800 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-white/[0.03] dark:hover:text-gray-200">
                <svg
                  className="fill-current"
                  width="20"
              ...
              <div className="mt-7">
                ...
                <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                  <div className="col-span-2 lg:col-span-1">
                    ...
                    <Input type="text" onChange={handleInputChange} name="first_name" defaultValue={profile?.user?.first_name}  />
                  </div>

                  <div className="col-span-2 lg:col-span-1">
                    <Label>Last Name</Label>
                    <Input type="text" onChange={handleInputChange} name="last_name" defaultValue={profile?.user?.last_name}  />
                  </div>
                  
                  {/* <div className="col-span-2 lg:col-span-1">
                    <Label>Email Address</Label>
                    <Input type="text" name="" defaultValue={profile?.user?.email} />
                  </div>

                  <div className="col-span-2 lg:col-span-1">
                    <Label>Phone</Label>
                    <Input type="text" defaultValue={profile?.user?.phone} />
                  </div> */}

                  <div className="col-span-2">
                    <Label>Bio</Label>
                    <Input type="text" onChange={handleInputChange} name="bio" defaultValue={profile?.user?.bio} />
                  </div>
                  <div className="col-span-2">
